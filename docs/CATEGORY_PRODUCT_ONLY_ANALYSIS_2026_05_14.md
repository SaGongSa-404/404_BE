...
- 분류 규칙: 상품명/설명/본문 텍스트 기반 키워드 규칙과 URL 보조 신호를 비교했다.
- 이 문서에 남아 있는 원본 근거: 사이트별 집계, 오답 샘플, 상품 상세 URL 미확보 샘플
- 현재 PR에 별도 파일로 남아 있지 않은 자료: 70개 전체 입력 샘플의 machine-readable 목록, 실행 명령, RAW 실행 결과 JSON/CSV
- 그래서 이 문서는 실험 결과 공유용으로는 사용할 수 있지만, 동일 결과를 재실행 검증하려면 전체 입력 샘플과 RAW 결과 파일을 별도 커밋해야 한다.
