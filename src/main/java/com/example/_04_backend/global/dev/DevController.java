...
public class DevController {
    ...
    /**
     * 로그인된 사용자에게 24시간이 지난 테스트 아이템을 생성한다.
     * 반환된 itemId로 구매 숙려 API를 바로 테스트할 수 있다.
     *
    ...
    public ResponseEntity<Map<String, String>> createTestWish(
        ...
        SavedItem item = SavedItem.builder()
                ...
                .inputSource(ItemInputSource.DIRECT_INPUT)
                .build();
        savedItemRepository.save(item);

        // JPA Auditing이 set한 created_at을 25시간 전으로 덮어써서 숙려 조건 통과
        em.createNativeQuery(
                "UPDATE saved_items SET created_at = :ts WHERE id = :id")
        ...
        return ResponseEntity.ok(Map.of(
                ...
                "tip", "GET /api/wishes/{itemId}/deliberation 으로 숙려 화면 조회 가능"
        ));
    }
}
