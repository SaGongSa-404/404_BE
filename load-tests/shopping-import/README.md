...
단일 reviewer 계정으로 비동기 테스트를 실행하기 전에는 다음 명령으로 QA의 사용자별 상한을 101로 임시 적용하고 QA 배포 workflow가 성공할 때까지 기다린다.
...
2. `Run NF-84 QA Load Test` workflow에 scenario, sync/async, 승인된 상품 URL을 입력한다. `backpressure + sync`는 reviewer 토큰 하나로 약 100개의 동기 크롤링 요청을 직접 발생시킨다.
